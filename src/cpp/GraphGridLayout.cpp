...
std::vector<uint64_t> GraphGridLayout::topoSort(LayoutState &state, uint64_t entry)
{
    ...
    auto dfsFragment = [&visited, &blocks, &state, &stack, &blockOrder](uint64_t first) {
                                                ...
                                                int &height) const
{
    ...
    for (auto &it : (*state.blocks)) {
        ...
        for (size_t i = 0; i < block.edges.size(); i++) {
            ...
            resultEdge.polyline.push_back(Point2f(0, block.y + block.height));
...
void GraphGridLayout::cropToContent(GraphLayout::Graph &graph, int &width, int &height) const
{
    ...
    for (auto &blockIt : graph) {
        ...
        for (auto &edge : block.edges) {
            for (auto &point : edge.polyline) {
                point -= Point2f(minPos[0], minPos[1]);
            }
        }
    }
